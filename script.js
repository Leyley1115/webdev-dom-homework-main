...
function answer(){
  ...
  for (const answerCom of commAll){
    ...
    index = answerCom.dataset.answer;
    const comment = commentBox[index];
    ...
    newTextValue = [comment.comment, comment.name];
    return newTextValue && index
    })
  }
}
...
const addAnswerFunc = () => {
  text.addEventListener("input", (event) => {
    addAnswer = event.target.value.replace(`${commentBox[index].comment.replaceAll(`<br>`, "")}`, "").replace(`${commentBox[index].name}`, "").replace(",", "").replace("     ", "").replace(">", "");
  });
}
addAnswerFunc();
